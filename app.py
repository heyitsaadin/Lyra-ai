...
def _try_huggingface(prompt):
    ...
    if not hf_token:
        print("[IMG] HF_TOKEN not set, skipping HuggingFace")
        return None
    models = [
    ...
    for model in models:
        try:
            ...
            print(f"[IMG][HF] {model} → {res.status_code} {ct[:40]} body[:80]={res.content[:80]}")
            if res.status_code == 200 and ct.startswith("image"):
            ...
            print(f"[IMG][HF] {model} exception: {e}")
    return None
...
{
  "action": "text" | "get_time" | "get_date" | "math" | "quiz_redirect" | "youtube_search" | "generate_image" | "edit_image",
