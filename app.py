...
def generate_image(prompt):
    """Return an HTML img tag using Pollinations.ai Flux model — photorealistic, no API key needed."""
    safe_prompt = quote(prompt)
    # model=flux gives photorealistic results; seed randomised for variety
    import random
    seed = random.randint(1, 99999)
    url = (
        f"https://image.pollinations.ai/prompt/{safe_prompt}"
        f"?width=768&height=768&nologo=true&enhance=true&model=flux&seed={seed}"
    )
