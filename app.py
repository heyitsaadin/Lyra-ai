...
TOPIC_KEYWORDS = {
    ...
    "tech":     ["ai","machine learning","neural","model","gpt","chatgpt","lyra","llm","data","server","cloud"],
...
def get_greeting(username):
    ...
    greetings = [
        ...
        f"Good {period}, {username}! ✨ Great to see you — what can Lyra do for you today?",
        f"Hey hey, {username}! 🌟 Good {period}! I'm all ears — what do you need?",
        f"Good {period}, {username}! 🤖 Lyra online and ready. What's up?",
    ]
...
def generate_image(prompt):
    ...
    return (
        ...
        f'<img src="{url}" alt="{prompt}" class="lyra-img" '
        ...
        f'<button onclick="downloadImage(\'{url}\',\'{escaped_prompt}\')" class="lyra-img-dl">'
        f'⬇ Download'
        f'</button>'
        f'</div>'
        f'</div>'
    ...
    try:
        ...
        if "data" in data and data["data"]:
            ...
            html = (
                ...
                f'<div class="lyra-img-footer">'
                f'<span class="lyra-img-caption">🎨 {prompt} ✨</span>'
                f'<button onclick="downloadImage(\'{src}\',\'{escaped_prompt}\')" class="lyra-img-dl">'
                f'⬇ Download'
                f'</button>'
                f'</div>'
                f'</div>'
...
def _compact_for_session(text):
    if 'lyra-img-wrap' in text:
        m = re.search(r'lyra-img-caption[^>]*>🎨\s*(.*?)(?:\s*✨\s*)?</span>', text)
...
def _build_clean_history(history):
    ...
    clean_msgs        = []
    # llama-3.1-8b-instant has ~8k token context. System prompt + response reserves ~1.5k tokens.
    # ~6.5k tokens left for history. 1 token ~ 4 chars, so budget = ~26k chars. Use 18k safely.
    MAX_HISTORY_CHARS = 18000
    total_chars = 0

    # First pass: scan ALL history to find last image prompt/analysis (no char limit)
    for msg in history:
        txt = msg["text"]
        if txt.startswith("[IMAGE GENERATED:"):
            last_img_prompt = txt[len("[IMAGE GENERATED:"):].rstrip("]").strip()
        elif 'lyra-img-wrap' in txt:
            m = re.search(r'lyra-img-caption[^>]*>🎨\s*(.*?)(?:\s*✨\s*)?</span>', txt)
    ...
    # Second pass: build clean messages within char budget from recent messages
    for msg in history[-30:]:
        txt = msg["text"]
        if txt.startswith("[IMAGE GENERATED:"):
            img_prompt = txt[len("[IMAGE GENERATED:"):].rstrip("]").strip()
            content = f"[I generated an image: {img_prompt}]"
            ...
            # Truncate heavily — full analysis is in last_img_analysis via system prompt
            analysis_short = txt[len("[IMAGE ANALYSIS RESULT]"):].strip()[:150]
            entry = {"role": "system", "content": "[Image description]: " + analysis_short}
            ...
            entry = {"role": role, "content": txt[:200]}
        else:
            role = "user" if msg["sender"] == "You" else "assistant"
            # Truncate very long individual messages to prevent single-message overflow
            content = txt[:600] if len(txt) > 600 else txt
            entry = {"role": role, "content": content}

        entry_len = len(entry["content"])
        if total_chars + entry_len > MAX_HISTORY_CHARS:
            break
...
def ask_lyra_brain(prompt, history=None):
    if history is None:
        history = []
    API_KEY = os.environ.get("GROQ_API_KEY")
    if not API_KEY:
    ...
    system_msg = """You are Lyra, a personal AI assistant created by Aadin. You have a warm, friendly, and engaging personality — like a smart best friend who genuinely enjoys helping.
...
IMAGE GENERATION RULES — read carefully:
- Set action to "generate_image" ONLY when the user has EXPLICITLY requested an image/picture/drawing/photo/art/illustration in their current message.
- Valid triggers: "generate an image of X", "draw X", "show me X", "create a picture of X", "make an image of X", "generate X" (when X is clearly a visual subject).
- NEVER set action to "generate_image" for: casual statements, location mentions, personal facts, questions, greetings, or anything where an image was not directly requested.
- If the last message in chat history was a generated image AND the user is asking to change/redo/update/edit it, set action to "edit_image" with a full new prompt combining the original + their requested changes.
- Set high_quality to true if the user asked for: high quality, realistic, photorealistic, 4k, 8k, detailed, cinematic, professional, masterpiece, hyper realistic, or similar quality keywords.

YOU MUST ALWAYS RESPOND IN THIS EXACT JSON FORMAT — no markdown, no explanation outside the JSON:
{
  "action": "text" | "generate_image" | "edit_image",
  "reply": "your conversational reply (used when action is text; can be empty string otherwise)",
...
def ask_lyra(prompt, history=None, wants_code=False):
    if history is None:
        history = []
    API_KEY = os.environ.get("GROQ_API_KEY")
    if not API_KEY:
...
- NEVER use code blocks for recipes, instructions, or any non-code content."""
    system_msg = f"""You are Lyra, a personal AI assistant created by Aadin. You have a warm, friendly, and engaging personality — like a smart best friend who genuinely enjoys helping.
...
def _build_reply(user_msg):
    ...
    if contains_bad_words(user_msg):
        send_discord_alert(session["user"], "Abusive language toward Lyra or Aadin", user_msg)
    ...
    _has_recent_image = any(
        'lyra-img-wrap' in m.get("text", "") or m.get("text", "").startswith("[IMAGE GENERATED:")
    ...
    if _is_img_question:
        ...
        while _waited < 8:
            ...
            time.sleep(0.5)
            _waited += 0.5
    decision = ask_lyra_brain(user_msg, session["messages"])
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            session["messages"] = [{"sender": "Lyra", "text": greeting}]
...
def send():
    ...
    session["messages"].append({"sender": "Lyra", "text": session_text})
    update_profile(session["user"], reply, "Lyra"))
...
def generate_image_route():
    ...
    session["messages"].append({"sender": "Lyra", "text": _compact_for_session(reply)})
    update_profile(session["user"], reply, "Lyra"))
    _msgs_copy = list(session["messages"])
    threading.Thread(
        target=_auto_analyse_generated_image,
...
def send_image():
    ...
    try:
        ...
        session["messages"].append({"sender": "Lyra", "text": stored_reply})
        update_profile(session["user"], user_label, "You")
        update_profile(session["user"], reply, "Lyra"))
        session.modified = True
...
def account_chats():
    ...
    for r in rows:
        ...
        if not preview:
            for m in msgs:
                raw = m.get("text", "")
                if (raw.startswith("[IMAGE ANALYSIS") or "lyra-img-wrap" in raw
...
def account_delete_chat(session_key):
    ...
    if session.get("chat_key") == session_key:
        session["messages"] = [{"sender": "Lyra", "text": get_greeting(session["user"])}]
        session["chat_key"] = secrets.token_hex(8)
        session.modified = True
    return jsonify({"ok": True}), 200
...
def share_chat():
    ...
    for m in msgs:
        ...
        if txt.startswith("[IMAGE GENERATED:"):
            ...
            txt = f'<div class="lyra-img-wrap"><img src="{img_url}" alt="{prompt_text}" class="lyra-img" onload="this.classList.add(\'loaded\')"><div class="lyra-img-footer"><span class="lyra-img-caption">🎨 {prompt_text} ✨</span></div></div>'
...
def admin_analyse_chats(username):
    ...
    sys_p = """You are an AI analyst reviewing aggregated usage metadata about a user of a chatbot called Lyra.
