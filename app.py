...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            session["session_id"]          = None   # lazy — created on first message
            greeting = get_greeting(username)
            session["messages"] = [{"sender": "Jarvis", "text": greeting}]
            # ✦ Do NOT save greeting or create session yet — only on first real message
...
def _ensure_session(user_msg):
    ...
    if not sid:
        ...
        # Persist the greeting that was shown but not yet saved
        msgs = session.get("messages", [])
        if msgs and msgs[0]["sender"] == "Jarvis":
            save_chat(session["user"], "Jarvis", msgs[0]["text"], sid)
    elif len(session.get("messages", [])) == 1:
        # Session exists but untitled — set title from first user message
        conn = get_db(); cur = conn.cursor()
...
def clear_all_chats():
    """Deletes all sessions and chats for the current user, then starts fresh."""
    if "user" not in session:
        import json
        return json.dumps({"error": "not_logged_in"}), 401, {"Content-Type": "application/json"}
    username = session["user"]
    ...
    conn.commit(); cur.close(); conn.close()
    # Reset in-memory session to fresh state
    session["session_id"] = None
    greeting = get_greeting(username)
    session["messages"] = [{"sender": "Jarvis", "text": greeting}]
    session.modified = True
