...
def _extract_pdf_text(pdf_file, max_pages=20, max_chars=4000):
...
7. Return exactly {count} questions."""
    ...
    try:
        ...
        # Handle Groq API-level errors (rate limit, bad request, etc.)
        if "error" in res_json:
            err_msg = res_json["error"].get("message", "Groq API error") if isinstance(res_json["error"], dict) else str(res_json["error"])
            return _json.dumps({"error": f"AI error: {err_msg}"}), 500, {"Content-Type": "application/json"}

        if "choices" not in res_json or not res_json["choices"]:
            return _json.dumps({"error": "AI returned an empty response. Please try again."}), 500, {"Content-Type": "application/json"}
...
Return ONLY the formatted exam paper text. No preamble, no markdown, no extra commentary."""
    ...
    try:
        ...
        exam_paper = res_json["choices"][0]["message"]["content"].strip()
