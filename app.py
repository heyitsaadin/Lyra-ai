...
def _extract_pdf_text(pdf_file, max_pages=15, max_chars=3000):
...
7. Return exactly {count} questions."""
    ...
    try:
        ...
        if "error" in res_json:
            err_msg = res_json["error"].get("message", str(res_json["error"])) if isinstance(res_json["error"], dict) else str(res_json["error"])
            return _json.dumps({"error": f"AI error: {err_msg}"}), 500, {"Content-Type": "application/json"}

        if "choices" not in res_json or not res_json["choices"]:
            return _json.dumps({"error": f"Unexpected API response: {str(res_json)[:200]}"}), 500, {"Content-Type": "application/json"}
...
def generate_exam():
    ...
    system_prompt = f"""You are an expert academic exam question writer.
Read the study material and return a JSON array of {count + 1} items.

DIFFICULTY: {diff_note}

CRITICAL RULES:
1. Return ONLY a valid JSON array. No preamble, no markdown fences, no explanation.
2. The FIRST item must be a meta object:
   {{"__meta__": true, "subject": "[infer a short subject name, e.g. Computer Science]"}}
3. The remaining {count} items are MCQ questions, each with EXACTLY these keys:
     "question"    : the full question string (a proper exam question)
     "options"     : array of exactly 4 strings
     "answer"      : the EXACT string from options that is correct
     "explanation" : 2-3 sentences explaining why the answer is correct
4. Use a variety of question types: recall, application, comparison, conceptual, why/how.
5. All 4 options must be specific and plausible. No "None of the above" or "All of the above".
6. Base all questions strictly on the provided material.
7. Return exactly {count} question objects plus 1 meta = {count + 1} total items."""
...
# ──────────────────────────────────────────────
# SEND IMAGE TO JARVIS (Gemini Vision)
# ──────────────────────────────────────────────
...
def send_image():
    ...
    Accepts a multipart POST with:
        ...
        caption — optional text from the user (e.g. "what's in this?")
    Sends it to Gemini 1.5 Flash for vision analysis.
    Returns Jarvis's reply as JSON.
    """
    ...
    if not img_file:
        return _json.dumps({"error": "No image received."}), 400, {"Content-Type": "application/json"}

    caption  = request.form.get("caption", "").strip()
    user_msg = caption if caption else "What's in this image?"
    ...
    system_context = """You are Jarvis, a warm and friendly AI assistant created by Aadin.
The user has sent you an image. Analyse it carefully and respond naturally.
- Be conversational and warm, not robotic.
- Use emojis naturally where fitting.
- Keep replies concise unless the user asks for detail.
- If the image contains text, read and share it accurately.
- If asked to edit or describe changes, explain what would change since you can only analyse, not modify images."""
    ...
    try:
        ...
        reply = "I had trouble reading that image — try again! 😅"

    # Save to session and DB
    sid = _ensure_session(user_msg)
    label = f"📷 {user_msg}"
    session["messages"].append({"sender": "You",    "text": label})
    session["messages"].append({"sender": "Jarvis", "text": reply})
    save_chat(session["user"], "You",    label, sid)
    save_chat(session["user"], "Jarvis", reply, sid)
    session.modified = True

    return _json.dumps({"reply": reply, "label": label, "session_id": sid}), 200, {"Content-Type": "application/json"}
