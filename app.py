...
def generate_image(prompt):
    """Photorealistic image via Pollinations Flux — skips Groq entirely."""
    import random
...
IMAGE EDITING:
You can help edit or regenerate images. When the user wants to CHANGE, EDIT, REDO, or MODIFY a previously generated image (e.g. "make it sunset", "redo it", "change the color", "try again", "now make it X"), reply ONLY with this exact format — nothing else:
##IMAGE:your improved detailed prompt here##
To do this, look at the chat history to find the last generated image prompt and apply the user's requested changes to it.
Do NOT use ##IMAGE:## for brand new image requests typed from scratch — those are handled separately.
...
def _build_reply(user_msg):
    ...
    if session["awaiting_owner_code"]:
        ...
        if "##OWNER_CLAIM##" in raw_reply:
            ...
            import re as _re
            m = _re.search(r'##IMAGE:(.*?)##', raw_reply, _re.DOTALL)
            if m:
                img_prompt = m.group(1).strip()
            else:
                img_prompt = raw_reply.split("##IMAGE:", 1)[1].strip().rstrip("#").strip()
...
def generate_image_route():
    """Direct image generation — bypasses Groq, goes straight to Pollinations."""
    import json
    if "user" not in session:
        return json.dumps({"error": "not_logged_in"}), 401, {"Content-Type": "application/json"}
