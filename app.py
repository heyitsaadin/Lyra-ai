...
def guest_entry():
    guest_sid   = secrets.token_hex(16)
    guest_label = get_next_guest_label()
    create_guest_session(guest_sid, guest_label)
    session.permanent = False
    session["guest_id"]          = guest_sid
    session["guest_label"]       = guest_label
    session["guest_msgs"]        = 0      # total user messages
    session["guest_imgs"]        = 0      # images generated
...
@app.route("/chat_guest")
def chat_guest():
    if "guest_id" not in session:
        return redirect("/signup")
    session.setdefault("messages", [])
    return render_template("chat_guest.html",
                           guest_label=session["guest_label"],
                           messages=session["messages"],
                           guest_imgs=session.get("guest_imgs", 0),
                           guest_msgs=session.get("guest_msgs", 0))

@app.route("/send_guest", methods=["POST"])
@limiter.limit("20 per minute")
@limiter.limit("4 per 10 seconds")
def send_guest():
    if "guest_id" not in session:
        return _json_mod.dumps({"error": "not_guest"}), 401, {"Content-Type": "application/json"}
    session.setdefault("messages", [])
    session.setdefault("guest_msgs", 0)
    session.setdefault("guest_imgs", 0)
    session.setdefault("guest_analyses", 0)
    session.setdefault("guest_edits", 0)
    session.setdefault("guest_quiz_done", False)
    ...
    # Hard message cap
    if session["guest_msgs"] >= GUEST_MSG_LIMIT:
        return _json_mod.dumps({"limit": "msgs", "reply": "__LIMIT_MSGS__"}), 200, {"Content-Type": "application/json"}

    # Quiz restriction (block quiz_redirect action — check intent via keyword)
    quiz_words = ["quiz", "exam", "test", "question", "mcq"]
    if any(w in user_msg.lower() for w in quiz_words) and session["guest_quiz_done"]:
        return _json_mod.dumps({"limit": "quiz", "reply": "You've already used the quiz feature. Create a free account to use it again! 🎓"}), 200, {"Content-Type": "application/json"}

    # Image generation cap check (checked before calling brain so we can intercept early)
    img_words = ["generate", "draw", "create", "make", "image", "picture", "photo", "art", "illustration", "paint"]
    _looks_like_img = any(w in user_msg.lower() for w in img_words)
    if _looks_like_img and session["guest_imgs"] >= GUEST_IMG_LIMIT:
        return _json_mod.dumps({"limit": "imgs", "reply": "__LIMIT_IMGS__"}), 200, {"Content-Type": "application/json"}

    session["messages"].append({"sender": "You", "text": user_msg})
    if len(session["messages"]) > 30:
        session["messages"] = session["messages"][-30:]

    # Temporarily spoof session["user"] so _build_reply helpers work
    # But skip DB writes — use a mock username
    _orig_user   = session.get("user")
    session["user"] = session["guest_label"]  # needed by _build_reply for math history etc.

    decision = ask_jarvis_brain(user_msg, session["messages"])
    action   = decision["action"]

    # Restore
    if _orig_user is None:
        session.pop("user", None)
    else:
        session["user"] = _orig_user
    ...
    elif action == "quiz_redirect":
        if session["guest_quiz_done"]:
            reply = "You've already used the quiz feature once. Create a free account to use it again! 🎓"
        else:
            session["guest_quiz_done"] = True
            reply = "📄 Upload a PDF on the <a href='/quiz' style='text-decoration:underline'>Quiz page</a> to generate questions! 🎓 (1 use per guest session)"
    elif action == "web_search":
        query  = decision["reply"].strip() or user_msg
        result = _do_google_search(query, user_msg=user_msg)
        reply  = result if result else "Couldn't find results for that."
    elif action == "youtube_search":
        query     = decision["reply"].strip() or user_msg
        plus_text = decision.get("youtube_plus_text", "").strip()
        yt_html   = search_youtube(query)
        reply     = f"{plus_text}##YT_SPLIT##{yt_html}" if plus_text else yt_html
    elif action in ("generate_image", "edit_image", "text") and "##IMAGE:" in decision.get("reply", ""):
        img_prompt = re.search(r'##IMAGE:(.*?)##', decision["reply"], re.DOTALL)
        img_prompt = img_prompt.group(1).strip() if img_prompt else user_msg
        if session["guest_imgs"] >= GUEST_IMG_LIMIT:
            reply = "__LIMIT_IMGS__"
        else:
            reply = generate_image(img_prompt)
            session["guest_imgs"] = session.get("guest_imgs", 0) + 1
    elif action in ("generate_image", "edit_image"):
        img_prompt = decision["image_prompt"].strip()
        if not img_prompt:
            reply = "Sure! Describe what image you'd like and I'll generate it 🎨"
        elif session["guest_imgs"] >= GUEST_IMG_LIMIT:
            reply = "__LIMIT_IMGS__"
        else:
            reply = generate_image(img_prompt)
            session["guest_imgs"] = session.get("guest_imgs", 0) + 1
    else:
        raw_reply = decision.get("reply", "")
        if "##OWNER_CLAIM##" in raw_reply:
            reply = "Identity verification is only available for registered users."
        elif "##SECURITY_BREACH##" in raw_reply:
            reply = "I don't have access to that information."
        else:
            reply = raw_reply or "I'm having a moment — try again! 😅"
    ...
    return _json_mod.dumps({
        "reply":     reply,
        "imgs_left": imgs_left,
        "msgs_left": max(0, GUEST_MSG_LIMIT - session["guest_msgs"])
    }), 200, {"Content-Type": "application/json"}

@app.route("/send_image_guest", methods=["POST"])
@limiter.limit("10 per minute")
def send_image_guest():
    if "guest_id" not in session:
        return jsonify({"error": "not_guest"}), 401
    session.setdefault("guest_analyses", 0)
    session.setdefault("guest_edits", 0)
    session.setdefault("messages", [])
    ...
    # Classify intent to know which counter to check
    groq_key = os.environ.get("GROQ_API_KEY_2", "") or os.environ.get("GROQ_API_KEY", "")
    import base64 as _b64
    ...
    if intent == "analyse" and session["guest_analyses"] >= GUEST_ANALYSIS_LIMIT:
        return jsonify({"limit": "analyses", "reply": "You've reached the guest image analysis limit (2). Create a free account for unlimited analysis! 🔍"}), 200
    if intent == "edit" and session["guest_edits"] >= GUEST_EDIT_LIMIT:
        return jsonify({"limit": "edits", "reply": "You've used your guest image edit. Create a free account for more! ✏️"}), 200
    ...
    try:
        if intent == "analyse":
            ...
            session["guest_analyses"] = session.get("guest_analyses", 0) + 1
        else:
            # Guests can't use NVIDIA edit — just tell them
            reply = "Image editing requires a free account. Sign up to unlock it! ✏️"
...
def account_chats():
    ...
    for r in rows:
        ...
        if not preview:
            for m in msgs:
                raw = m.get("text", "")
                if (raw.startswith("[IMAGE ANALYSIS") or "jarvis-img-wrap" in raw or "mia-img-wrap" in raw
...
def debug_wiki():
    ...
    try:
        ...
        return jsonify({"status": resp.status_code, "items": len(resp.json().get("query", {}).get("search", []))})
    except Exception as e:
...
def debug_search():
    ...
    try:
        resp = requests.get(
            ...
            params={"key": api_key, "cx": cse_id, "q": "gold rate india", "num": 4},
            timeout=10
        )
        return jsonify({
            "status": resp.status_code,
            "api_key_set": bool(api_key),
            "api_key_preview": api_key[:8] + "..." if api_key else "EMPTY",
            "cse_id": cse_id,
            "response": resp.json()
        })
    except Exception as e:
