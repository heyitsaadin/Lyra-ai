...
def generate_image(prompt):
    """Return an HTML img tag using Pollinations.ai Flux (photorealistic) — no API key needed."""
    import random
    safe_prompt = quote(prompt)
    seed = random.randint(1, 99999)
    url = (
        "https://image.pollinations.ai/prompt/" + safe_prompt
        + "?width=768&height=768&nologo=true&enhance=true&model=flux&seed=" + str(seed)
    )
    ep = prompt.replace("'", "\\'")
    parts = [
        '<div class="jarvis-img-wrap">',
        '<img src="', url, '" alt="', prompt, '" class="jarvis-img" ',
        "onload=\"this.classList.add('loaded')\" ",
        "onerror=\"this.parentElement.innerHTML='&#10060; Could not generate image. Try a different prompt.'\">",
        '<div class="jarvis-img-footer">',
        '<span class="jarvis-img-caption">\U0001f3a8 ', prompt, '</span>',
        "<button onclick=\"downloadImage('", url, "','", ep, "')\" class=\"jarvis-img-dl\">",
        '\u2b07 Download',
        '</button>',
        '</div>',
        '</div>',
...
IMAGE GENERATION:
You have the ability to generate and edit images. When the user asks for an image (new or edited), or wants to change/redo/modify a previously generated image, reply ONLY with this exact format and nothing else:
##IMAGE:your detailed photorealistic prompt here##
- Always write a rich, descriptive prompt (include style, lighting, details).
- If the user wants to EDIT or REDO a previous image, look at the chat history to understand what was generated and apply their requested changes to that prompt.
- If the user says things like "redo it", "try again", "make it X instead", "change it to", "now make it" — these are image edit requests. Use the last image context from history.
- Do NOT add any extra text outside the ##IMAGE:...## tag for image requests.
...
def _build_reply(user_msg):
    ...
    if session["awaiting_owner_code"]:
        ...
        elif raw_reply.strip().startswith("##IMAGE:") and raw_reply.strip().endswith("##"):
            # Groq detected an image request or edit — extract prompt and generate
            img_prompt = raw_reply.strip()[8:-2].strip()
