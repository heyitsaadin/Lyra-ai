...
TOPIC_KEYWORDS = {
    ...
    "tech":     ["ai","machine learning","neural","model","gpt","chatgpt","rakai","llm","data","server","cloud"],
...
def get_greeting(username):
    ...
    greetings = [
        ...
        f"Good {period}, {username}! ✨ Great to see you — what can Rakai do for you today?",
        f"Hey hey, {username}! 🌟 Good {period}! I'm all ears — what do you need?",
        f"Good {period}, {username}! 🤖 Rakai online and ready. What's up?",
    ]
...
def ask_jarvis_brain(prompt, history=None):
    ...
    system_msg = """You are Rakai, a personal AI assistant created by Aadin. You have a warm, friendly, and engaging personality — like a smart best friend who genuinely enjoys helping.
...
- NEVER use code blocks for recipes, instructions, or any non-code content."""
    system_msg = f"""You are Rakai, a personal AI assistant created by Aadin. You have a warm, friendly, and engaging personality — like a smart best friend who genuinely enjoys helping.
...
def _build_reply(user_msg):
    ...
    if contains_bad_words(user_msg):
        send_discord_alert(session["user"], "Abusive language toward Rakai or Aadin", user_msg)
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            session["messages"] = [{"sender": "Rakai", "text": greeting}]
...
def send():
    ...
    session["messages"].append({"sender": "Rakai", "text": session_text})
    update_profile(session["user"], reply, "Rakai")
...
def generate_image_route():
    ...
    session["messages"].append({"sender": "Rakai", "text": _compact_for_session(reply)})
    update_profile(session["user"], "image_generated", "Rakai")
    _msgs_copy = list(session["messages"])
    threading.Thread(
        target=_auto_analyse_generated_image,
...
def send_image():
    ...
    try:
        ...
        session["messages"].append({"sender": "Rakai", "text": stored_reply})
        update_profile(session["user"], user_label, "You")
        update_profile(session["user"], "image_analysed_or_edited", "Rakai")
        session.modified = True
...
def account_delete_chat(session_key):
    ...
    if session.get("chat_key") == session_key:
        session["messages"] = [{"sender": "Rakai", "text": get_greeting(session["user"])}]
        session["chat_key"] = secrets.token_hex(8)
        session.modified = True
    return jsonify({"ok": True}), 200
...
def admin_analyse_chats(username):
    ...
    sys_p = """You are an AI analyst reviewing aggregated usage metadata about a user of a chatbot called Rakai.
