...
def generate_image_hf(prompt):
    """Hugging Face Inference API — free Pollinations replacement."""
    import base64 as _b64
...
def generate_image(prompt):
    import base64 as _b64
    # Try Hugging Face first (free, returns base64 - no external URL in browser)
    hf_result = generate_image_hf(prompt)
    if hf_result:
        return hf_result
    # Fallback: fetch Pollinations SERVER-SIDE and return base64
    # Never expose a Pollinations URL directly to the browser
    try:
        safe_prompt = quote(prompt)
        pol_url = f"https://image.pollinations.ai/prompt/{safe_prompt}?width=768&height=768&nologo=true&enhance=true"
        resp = requests.get(pol_url, timeout=45)
        if resp.status_code == 200 and resp.headers.get("content-type", "").startswith("image"):
            img_bytes = compress_image(resp.content)
            b64 = _b64.b64encode(img_bytes).decode("utf-8")
            src = f"data:image/jpeg;base64,{b64}"
            escaped_prompt = prompt.replace("'", "\\'")
            return (
        ...
        pass
    return "❌ Could not generate image. Please try again with a different prompt."
