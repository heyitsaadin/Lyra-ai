...
def save_chat(username, sender, message):
    ...
    cur.execute("INSERT INTO chats VALUES (%s, %s, %s, %s)", (username, sender, message, timestamp))
    conn.commit()
    cur.close(); conn.close()
...
def init_db():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS chats   (username TEXT, sender TEXT, message TEXT, timestamp TEXT)")
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            save_chat(username, "Jarvis", greeting)
...
def chat():
    ...
    if request.method == "POST":
        ...
        save_chat(session["user"], "You", user_msg)
