...
def get_chat_sessions(username):
    """Return list of saved chat sessions for a user (newest first)."""
    conn = get_db(); cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute(
        "SELECT id, session_key, messages, created_at, updated_at FROM chat_sessions "
...
def check_ban():
    ...
    exempt = ["/admin", "/login", "/", "/signup", "/static"]
...
def account_chats():
    ...
    for r in rows:
        ...
        # Use first user message as preview (skip Jarvis greeting)
        preview = ""
        for m in msgs:
            if m.get("sender") == "You":
                ...
                break
        if not preview:
            preview = (msgs[0].get("text", "") if msgs else "")[:60]
        chats.append({
            "id": r["id"],
...
# ── Delete account (used by both chat.html and landing.html) ──

@app.route("/account/delete", methods=["POST"])
@app.route("/delete_account", methods=["POST"])
@limiter.limit("5 per minute")
def account_delete():
    """Permanently delete the logged-in user's account after password verification."""
    import json
    if "user" not in session:
        return json.dumps({"ok": False, "error": "Not logged in"}), 401, {"Content-Type": "application/json"}
    ...
    user = get_user(session["user"])
    if not user or not check_password_hash(user["password"], password):
        return json.dumps({"ok": False, "error": "Incorrect password"}), 200, {"Content-Type": "application/json"}
...
# ── Get sessions (used by landing.html's chat history panel) ──

@app.route("/get_sessions")
def get_sessions():
    """Return chat sessions in the format expected by landing.html."""
    import json
    if "user" not in session:
        return json.dumps({"error": "not_logged_in"}), 401, {"Content-Type": "application/json"}
    rows = get_chat_sessions(session["user"])
    ...
    for r in rows:
        ...
        for m in msgs:
            if m.get("sender") == "You":
                ...
                break
        if not title:
            title = (msgs[0].get("text", "") if msgs else "")[:60]
        sessions_out.append({
    ...
    return json.dumps({"sessions": sessions_out}), 200, {"Content-Type": "application/json"}
